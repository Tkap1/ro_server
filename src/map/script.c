...
static BUILDIN(setequipoption)
{
	...
	if (sd->status.inventory[i].nameid != 0) {
		...
		int index_before = sd->status.inventory[i].equip;
		pc->unequipitem(sd, i, PCUNEQUIPITEM_FORCE); // status calc will happen in pc->equipitem() below
		...
		pc->equipitem(sd, i, index_before);	// Tkap: fixed this line, it was using the position index thing of the item
		// you are adding the option to AFTER it was already unequipped, instead of getting it before
